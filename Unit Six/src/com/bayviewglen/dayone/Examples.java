...
public class Examples {
	...
	public static void main(String[] args){
		...
		System.out.println(testScores[5]); // displays what is in the sixth element - should be 0.0
		// By default if you do not initialize a number it is always 0 
		
		// let's traverse the array - go through and hit every element
		// another word for this is iterate the array
		
		// there are NO METHODS for an array because it is not a class/object
		// but htere is one property called length - in this case 10
		
		for (int i = 0; i < testScores.length; i++){
			System.out.println("Element at index " + i + " - " + testScores[i]);
		}
		
		// create and array of primitives and initialize in one step
		...
		// spit out all of the words with the letter t/T in them
		for (int i = 0; i<words.length; i++){
	...
	public static double testAvergae(int[] array){
		int sum = 0;
		for (int i=0; i<array.length; i++){
			sum += array[i];
		}
